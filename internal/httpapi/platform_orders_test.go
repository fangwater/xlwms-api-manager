...
func TestPlatformOrderRoutingPreviewUsesExistingWarehouseMapping(t *testing.T) {
	source := readyPlatformOrderOperator()
	handler := NewWithPlatformOrderOperations(nil, nil, nil, source, readyPlatformMappings(), time.Second, slog.Default())
...
func TestAssignAndApproveRejectsUnmappedPlatformWarehouse(t *testing.T) {
	source := readyPlatformOrderOperator()
	handler := NewWithPlatformOrderOperations(nil, nil, nil, source, &fakePlatformMappings{}, time.Second, slog.Default())
	request := httptest.NewRequest(http.MethodPost, "/v1/platform-orders/assign-and-approve", strings.NewReader(operationRequestBody))
	recorder := httptest.NewRecorder()
	handler.ServeHTTP(recorder, request)
	if recorder.Code != http.StatusConflict {
	...
	if !strings.Contains(recorder.Body.String(), "尚未配置映射") {
		t.Fatalf("unexpected response: %s", recorder.Body.String())
	}
}

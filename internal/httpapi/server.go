...
func newWithPlatformOrderAccountOperationsAuthenticated(destination *store.Postgres, service *syncer.Service, fulfillmentAuditor *auditor.Service, platformOrders platformOrderSource, platformMappings platformWarehouseMappingSource, platformFulfillment platformOrderFulfillmentSource, platformAccounts platformOrderAccountSource, consoleUser, consolePassword string, requestTimeout time.Duration, logger *slog.Logger) http.Handler {
	...
	mux.HandleFunc("POST /v1/platform-sku-mappings", server.savePlatformSKUMapping)
	mux.HandleFunc("PUT /v1/platform-sku-mappings", server.savePlatformSKUMapping)
	...
	mux.HandleFunc("GET /v1/packing/combinations", server.listSKUCombinations)
	mux.HandleFunc("POST /v1/packing/combinations", server.createSKUCombination)
	mux.HandleFunc("GET /v1/packing/combinations/{id}", server.getSKUCombination)
	...
	mux.HandleFunc("POST /v1/inventory-thresholds/defaults/reset", inventoryThresholdDefaultResetGone)
	...
	mux.HandleFunc("GET /v1/fulfillment-policies/carriers", server.listCarrierPolicies)
	mux.HandleFunc("PUT /v1/fulfillment-policies/carriers/{warehouseKey}", server.updateCarrierPolicies)
	mux.HandleFunc("PATCH /v1/fulfillment-policies/carriers/{warehouseKey}", server.updateCarrierPolicies)
	mux.HandleFunc("POST /v1/fulfillment-policies/carriers/{warehouseKey}/reset", server.resetSKUCarrierPolicies)
	mux.HandleFunc("GET /v1/fulfillment-policies/skus", server.listSKUFulfillmentPolicies)

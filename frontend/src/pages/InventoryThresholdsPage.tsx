...
function stockClass(available: number, threshold: number, inclusive = false): string {
  return (inclusive ? available <= threshold : available < threshold) ? "danger" : "positive";
}
...
export default function InventoryThresholdsPage() {
  ...
  return <>
    ...
    {loading ? <LoadingState label="正在加载库存安全线" /> : data?.records.length ? <div className="table-panel"><div className="table-scroll"><table className="data-table threshold-table"><thead><tr><th>仓库 SKU / 产品</th><th>美东库存</th><th>美西库存</th><th>库存合计</th><th>转人工阈值</th><th>规则来源</th><th>操作</th></tr></thead><tbody>{data.records.map((item) => <tr key={item.warehouse_sku}><td><div className="primary-cell"><strong>{item.warehouse_sku}</strong><small>{item.product_name || "-"}</small></div></td><td>{item.inventory_at ? item.east_available : "-"}</td><td>{item.inventory_at ? item.west_available : "-"}</td><td className={item.inventory_at ? stockClass(item.total_available, item.total_threshold, item.total_inclusive) : ""}>{item.inventory_at ? item.total_available : "-"}</td><td>{item.total_inclusive ? "≤ " : "< "}{item.total_threshold}{item.warehouse_codes?.length ? ` · ${item.warehouse_codes.length} 仓` : ""}</td><td><div className="threshold-source"><span className={`status-badge ${item.customized ? "running" : ""}`}>{sourceLabel(item)}</span><small>{item.inventory_at ? `库存 ${dateTime(item.inventory_at)}` : "暂无库存快照"}</small></div></td><td><button className="icon-button bordered" disabled={item.source === "oms_account"} onClick={() => openEditor(item)} title="编辑安全线"><Pencil size={15}/></button></td></tr>)}</tbody></table></div></div> : <EmptyState label="当前没有仓库 SKU" />}

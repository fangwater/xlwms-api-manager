...
export default function AccountManagementView() {
  ...
  const toggleDraftWarehouse = (warehouseCode: string) => {
    setDraft((current) => ({
      ...current,
      warehouse_codes: current.warehouse_codes.includes(warehouseCode)
        ? current.warehouse_codes.filter((code) => code !== warehouseCode)
        : [...current.warehouse_codes, warehouseCode].sort()
    }));
  };
  ...
  return <div className="policy-view-content account-management-view">
    ...
    {creating && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setCreating(false); }}><section className="modal account-create-modal" role="dialog" aria-modal="true" aria-labelledby="account-create-title"><header><div><h2 id="account-create-title">新建 OMS 发货账户</h2><p>验证登录后保存</p></div><button className="icon-button" type="button" title="关闭" onClick={() => setCreating(false)}><X size={19}/></button></header><form onSubmit={createAccount}>{error && <div className="error-banner">{error}</div>}<div className="form-grid"><label><span>账户标识</span><input required autoFocus pattern="[a-z0-9][a-z0-9_-]{0,63}" placeholder="例如 backup" value={draft.key} onChange={(event) => setDraft({ ...draft, key: event.target.value.toLowerCase() })}/></label><label><span>显示名称</span><input required maxLength={100} placeholder="例如 备用账户" value={draft.label} onChange={(event) => setDraft({ ...draft, label: event.target.value })}/></label><label><span>OMS 账号</span><input required autoComplete="off" value={draft.username} onChange={(event) => setDraft({ ...draft, username: event.target.value })}/></label><label><span>OMS 密码</span><input required type="password" autoComplete="new-password" value={draft.password} onChange={(event) => setDraft({ ...draft, password: event.target.value })}/></label></div><div className="account-create-warehouses"><span>可操作仓库</span><div className="account-warehouse-grid">{warehouses.map((warehouse) => { const active = draft.warehouse_codes.includes(warehouse.wh_code); return <label className={active ? "active" : ""} key={warehouse.wh_code}><input type="checkbox" checked={active} onChange={() => toggleDraftWarehouse(warehouse.wh_code)}/><span>{active && <Check size={13}/>}</span><div><strong>{warehouse.wh_code}</strong><small>{warehouse.name || "仓库"}</small></div></label>; })}</div></div><div className="security-note"><ShieldCheck size={17}/><span>登录凭据验证通过后加密存储</span></div><footer><button className="secondary-button" type="button" onClick={() => setCreating(false)}>取消</button><button className="primary-button" type="submit" disabled={saving === "create"}>{saving === "create" ? "验证中" : "验证并新建"}</button></footer></form></section></div>}

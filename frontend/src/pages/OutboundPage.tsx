...
type OfficialPage = { records: OutboundOrder[]; total: number; page: number; pageSize: number; pages?: number; lastQueryAt?: string };
type IndexedPage = { records: OutboundOrder[]; total: number; page: number; page_size: number; pages: number; last_query_at?: string };
...
export default function OutboundPage({ warehouse }: { warehouse: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (mode === "parcel" && submittedQuery) {
        const result = await api.outboundOrders<IndexedPage>({ warehouse, q: submittedQuery, page, pageSize: 30 });
        setData({ ...result, pageSize: result.page_size, lastQueryAt: result.last_query_at });
      } else {
  ...
  return <>
    <PageHeader title="出库管理" subtitle={`小包与备货中转出库单${data?.lastQueryAt ? ` · 最近查询 ${dateTime(data.lastQueryAt)}` : ""}`} actions={<button className="icon-button bordered" onClick={() => void load()} title="刷新"><RefreshCw size={18} className={loading ? "spin" : ""} /></button>} />

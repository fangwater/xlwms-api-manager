import { ListOrdered, PackageSearch, ShieldCheck, UsersRound } from "lucide-react";
...
export type ShippingPolicyView = "base-rules" | "selection" | "sku-rules" | "accounts";
...
const views = [
  ...
  { key: "accounts", path: "/shipping-policies/accounts", label: "账户路由", title: "OMS 账户路由", subtitle: "按平台和仓库 SKU 确定发货账户，并维护账户可操作仓库", scope: "平台 + SKU", icon: UsersRound }
] as const;
...
export default function ShippingPoliciesPage({ view, onNavigate }: { view: ShippingPolicyView; onNavigate: (path: string) => void }) {
  ...
  return <>
    ...
    {view === "accounts" ? <AccountRoutesView platform={platform} platformLabel={platformLabel(platform)} /> : view === "sku-rules" ? <SKUShippingRulesView platform={platform} platformLabel={platformLabel(platform)} /> : <WarehouseRulesView platform={platform} mode={view} />}
  </>;
}

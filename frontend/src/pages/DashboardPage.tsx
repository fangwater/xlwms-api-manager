...
export default function DashboardPage({ warehouse, warehouses }: { warehouse: string; warehouses: Warehouse[] }) {
  ...
  if (error || !data) return <><PageHeader title="运营总览" /><ErrorState message={error || "暂无看板数据"} onRetry={() => void load()} /></>;
  const metrics = [
  ...
  return <>
    <PageHeader title="运营总览" subtitle={warehouse ? selectedWarehouseName : undefined} actions={<button className="icon-button bordered" onClick={() => void load()} title="刷新"><RefreshCw size={18} /></button>} />
